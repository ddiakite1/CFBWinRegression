...
secRushTrend <- sec %>%
  ggplot(aes(x = year, y = r_att, color = school)) +
  geom_point_interactive(aes(tooltip = sec$r_att)) +
  geom_line() +
  labs(x = "Year", y = "Avg. Rushing Attempts per Game")

girafe(ggobj = secRushTrend)

saveRDS(sec, file = "SEC.RDS")
saveRDS(b1g, file = "B1G.RDS")
saveRDS(b12, file = "B12.RDS")
saveRDS(pac, file = "PAC.RDS")
saveRDS(acc, file = "ACC.RDS")

# Summarized Rushing and Passing data by conference
secSum <- sec %>% group_by(year) %>% summarise(rushATT = mean(r_att), rushYDS = mean(r_yds),
                                               passATT = mean(p_att), passYDS = mean(p_yds),
                                               passCMP = mean(p_cmp), passPCT = mean(p_pct),
                                               rushTD = mean(r_td), passTD = mean(p_td))
b1gSum <- b1g %>% group_by(year) %>% summarise(rushATT = mean(r_att), rushYDS = mean(r_yds),
                                               passATT = mean(p_att), passYDS = mean(p_yds),
                                               passCMP = mean(p_cmp), passPCT = mean(p_pct),
                                               rushTD = mean(r_td), passTD = mean(p_td))
b12Sum <- b12 %>% group_by(year) %>% summarise(rushATT = mean(r_att), rushYDS = mean(r_yds),
                                               passATT = mean(p_att), passYDS = mean(p_yds),
                                               passCMP = mean(p_cmp), passPCT = mean(p_pct),
                                               rushTD = mean(r_td), passTD = mean(p_td))
pacSum <- pac %>% group_by(year) %>% summarise(rushATT = mean(r_att), rushYDS = mean(r_yds),
                                               passATT = mean(p_att), passYDS = mean(p_yds),
                                               passCMP = mean(p_cmp), passPCT = mean(p_pct),
                                               rushTD = mean(r_td), passTD = mean(p_td))
accSum <- acc %>% group_by(year) %>% summarise(rushATT = mean(r_att), rushYDS = mean(r_yds),
                                               passATT = mean(p_att), passYDS = mean(p_yds),
                                               passCMP = mean(p_cmp), passPCT = mean(p_pct),
                                               rushTD = mean(r_td), passTD = mean(p_td))

saveRDS(secSum, file = "secSum.RDS")
saveRDS(b1gSum, file = "b1gSum.RDS")
saveRDS(b12Sum, file = "b12Sum.RDS")
saveRDS(pacSum, file = "pacSum.RDS")
saveRDS(accSum, file = "accSum.RDS")
...
p5sum <- p5 %>% group_by(year, conference) %>% 
  summarise(rushATT = mean(r_att), rushYDS = mean(r_yds), passATT = mean(p_att), 
            passYDS = mean(p_yds), passCMP = mean(p_cmp), passPCT = mean(p_pct),
            rushTD = mean(r_td), passTD = mean(p_td))
...
secCor <- c(cor(sec$r_att, sec$c_w_pct), cor(sec$r_yds, sec$c_w_pct), cor(sec$p_att, sec$c_w_pct),
            cor(sec$p_yds, sec$c_w_pct), cor(sec$p_cmp, sec$c_w_pct), cor(sec$p_pct, sec$c_w_pct),
            cor(sec$r_td, sec$c_w_pct), cor(sec$p_td, sec$c_w_pct))
b1gCor <- c(cor(b1g$r_att, b1g$c_w_pct), cor(b1g$r_yds, b1g$c_w_pct), cor(b1g$p_att, b1g$c_w_pct),
            cor(b1g$p_yds, b1g$c_w_pct), cor(b1g$p_cmp, b1g$c_w_pct), cor(b1g$p_pct, b1g$c_w_pct),
            cor(b1g$r_td, b1g$c_w_pct), cor(b1g$p_td, b1g$c_w_pct))
b12Cor <- c(cor(b12$r_att, b12$c_w_pct), cor(b12$r_yds, b12$c_w_pct), cor(b12$p_att, b12$c_w_pct),
            cor(b12$p_yds, b12$c_w_pct), cor(b12$p_cmp, b12$c_w_pct), cor(b12$p_pct, b12$c_w_pct),
            cor(b12$r_td, b12$c_w_pct), cor(b12$p_td, b12$c_w_pct))
pacCor <- c(cor(pac$r_att, pac$c_w_pct), cor(pac$r_yds, pac$c_w_pct), cor(pac$p_att, pac$c_w_pct),
            cor(pac$p_yds, pac$c_w_pct), cor(pac$p_cmp, pac$c_w_pct), cor(pac$p_pct, pac$c_w_pct),
            cor(pac$r_td, pac$c_w_pct), cor(pac$p_td, pac$c_w_pct))
accCor <- c(cor(acc$r_att, acc$c_w_pct), cor(acc$r_yds, acc$c_w_pct), cor(acc$p_att, acc$c_w_pct),
            cor(acc$p_yds, acc$c_w_pct), cor(acc$p_cmp, acc$c_w_pct), cor(acc$p_pct, acc$c_w_pct),
            cor(acc$r_td, acc$c_w_pct), cor(acc$p_td, acc$c_w_pct))
p5Cor <- c(cor(p5$r_att, p5$w_pct), cor(p5$r_yds, p5$w_pct), cor(p5$p_att, p5$w_pct),
            cor(p5$p_yds, p5$w_pct), cor(p5$p_cmp, p5$w_pct), cor(p5$p_pct, p5$w_pct),
            cor(p5$r_td, p5$w_pct), cor(p5$p_td, p5$w_pct))

secCorgt <- data.frame(statistic, secCor) %>% arrange(desc(secCor)) %>% gt() %>% 
  tab_header(title = "Correlation To Conference Win Percentage", 
             subtitle = "In the Southeastern Conference") %>% 
  cols_label(statistic = "Statistical Category", secCor = "Correlation Coefficient")

b1gCorgt <- data.frame(statistic, b1gCor) %>% arrange(desc(b1gCor)) %>% gt() %>% 
  tab_header(title = "Correlation To Conference Win Percentage", 
             subtitle = "In the Big Ten Conference") %>% 
  cols_label(statistic = "Statistical Category", b1gCor = "Correlation Coefficient")

b12Corgt <- data.frame(statistic, b12Cor) %>% arrange(desc(b12Cor)) %>% gt() %>% 
  tab_header(title = "Correlation To Conference Win Percentage", 
             subtitle = "In the Big 12 Conference") %>% 
  cols_label(statistic = "Statistical Category", b12Cor = "Correlation Coefficient")

pacCorgt <- data.frame(statistic, pacCor) %>% arrange(desc(pacCor)) %>% gt() %>% 
  tab_header(title = "Correlation To Conference Win Percentage", 
             subtitle = "In the Pacific-12 Conference") %>% 
  cols_label(statistic = "Statistical Category", pacCor = "Correlation Coefficient")

accCorgt <- data.frame(statistic, accCor) %>% arrange(desc(accCor)) %>% gt() %>% 
  tab_header(title = "Correlation To Conference Win Percentage", 
             subtitle = "In the Atlantic Coast Conference") %>% 
  cols_label(statistic = "Statistical Category", accCor = "Correlation Coefficient")
...
saveRDS(secCorgt, file = "secCorgt.RDS")
saveRDS(b1gCorgt, file = "b1gCorgt.RDS")
saveRDS(b12Corgt, file = "b12Corgt.RDS")
saveRDS(pacCorgt, file = "pacCorgt.RDS")
saveRDS(accCorgt, file = "accCorgt.RDS")
